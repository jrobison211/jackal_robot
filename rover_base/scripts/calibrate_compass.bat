...
set TEMPDIR="%temp%\rover.calibrate_compass"
if exist %TEMPDIR% (
  if exist %TEMPDIR%\nul (
    rd /s /q %TEMPDIR%
  )
...
set CAL_FINAL_PATH="%ROS_ETC_DIR%\rover_base"
set DURATION=60
...
call rosrun rover_base compute_calibration %BAG_FILE% %CAL_FILE% >%temp%\compute_output.log 2>&1
if not "%errorlevel%" == "0" (
